...
for frame in range(n_frames):
    ...
    if len(history_ball_1) >= minimum_req:
        predicted_positions_ball_1 = predictors.bezier_curve_predictor(np.array(history_ball_1).tolist(), prediction_range)

    if len(history_ball_2) >= minimum_req:
        predicted_positions_ball_2 = predictors.bezier_curve_predictor(np.array(history_ball_2).tolist(), prediction_range)
    ...
    if predicted_positions_ball_1 and predicted_positions_ball_2:
        for p1 in predicted_positions_ball_1:
            for p2 in predicted_positions_ball_2:
                if (45 <= p1[0] <= 55 and 245 <= p1[1] <= 255):
