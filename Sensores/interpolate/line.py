...
def interpolate_missing(history):
    ...
    it falls back to returning None for those points.
    ...
    try:
        spline_x = UnivariateSpline(indices, x_coords, k=3, s=0.5)
        spline_y = UnivariateSpline(indices, y_coords, k=3, s=0.5)
    except Exception as e:
        print(f"Spline fitting failed: {e}")
        return history  # Return as is if fitting fails
...
for frame in range(n_frames):
    ...
    if len(history_ball_1) >= minimum_req:
        ...
        smoothed_history_ball_1 = [p for p in interpolated_history_ball_1 if p is not None]
    ...
    if len(history_ball_2) >= minimum_req:
        ...
        smoothed_history_ball_2 = [p for p in interpolated_history_ball_2 if p is not None]
